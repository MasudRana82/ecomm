    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm text-center">
            <a href="{{ route('category.show', $category->slug) }}">
                @if($category->image)
                    <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="mx-auto h-24 mb-2 object-cover">
                @else
                    <img src="https://placehold.co/200x150/FEF3C7/8B5CF6?text={{ urlencode($category->name) }}" class="mx-auto h-24 mb-2">
                @endif
    ...
    <div id="product-grid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden group text-center relative">
            ...
            <div class="p-4">
                ...
                <button class="mt-3 w-full bg-custom-orange text-white py-2 rounded-md hover:bg-custom-orange-dark transition-colors duration-300 add-to-cart" 
